...
## Estado actual · 0.2.0-rc.2 con CI verificada

**Candidata publicada:** [PR #2](https://github.com/Buzo500/atlas-quant/pull/2) fusionada y etiqueta [v0.2.0-rc.2](https://github.com/Buzo500/atlas-quant/tree/v0.2.0-rc.2) sobre `9aee4224a12556e97cde16ed283854af10845bda`. La [CI de la etiqueta, 34251099444](https://github.com/Buzo500/atlas-quant/actions/runs/34251099444), pasó sobre ese commit: 446 pruebas Python y 91 subtests, 140 de frontend y cinco E2E. Antes de fusionar pasó la [ejecución 34249730107](https://github.com/Buzo500/atlas-quant/actions/runs/34249730107), sobre `412918b5e9067e44f293b0633068ca932a472d64`; el cierre intermedio solo cambió documentación. Ambas validaciones se mantuvieron dentro de la cuota gratuita. El rechazo inicial de la revisión automática se conserva como antecedente, resuelto mediante autorización expresa.

**ATLAS está arrancado**, ejecución `7fa354c2b8d5425fa4818dab68acd8a0`, con salud rc.2, cartera y controles conservados, tres posiciones, NAV 25.118,66876 EUR, parada global activada y presupuesto/gasto/reserva cero, sin claves. El escalado físico 125 %/150 % está comprobado en este sobremesa; el ensayo de 48 horas permanece aplazado y la candidata no es una versión estable.

**Intento fallido conservado:** [CI 34248790750](https://github.com/Buzo500/atlas-quant/actions/runs/34248790750), sobre `dff5e24a0ebbce8a1cb0481fd64e47563794e433`: 444 pruebas Python superadas, dos fallidas y 91 subtests en 21,99 s; 140 Vitest superadas, sin E2E remoto acreditado en ese intento. Las dos variantes del test de aislamiento comparaban textualmente el alias corto `RUNNER~1` y la ruta larga `runneradmin`, aunque identificaban el mismo archivo. Se corrigió solo esa aserción mediante `samefile`, manteniendo la comprobación de aislamiento. La reproducción con ruta corta produjo dos fallos antes y dos casos correctos después; las dos variantes con ruta larga también pasan. Se conservan los logs y el resultado fallido.

Estado del 8 de septiembre de 2026. Motor e interfaz usan **`0.2.0-rc.2`**, desarrollada en `codex/v0.2.0-rc.2` sobre la [PR #1](https://github.com/Buzo500/atlas-quant/pull/1) fusionada en `master` (`e1f6e020a1d75a81bff97eefcbebe726d47bcdb3`). La candidata rc.1 y su trazabilidad permanecen como antecedentes.
...
| Conservación de la base habitual | En la validación previa a la publicación: integridad y contenido completo, esquema, secuencias y 539 eventos de auditoría iguales a `backups/atlas-20260908T151716582043Z-af195afe`. El run E2E correcto registra igualdad de hashes antes/después. La diferencia de archivos WAL/SHM del primer intento tiene una reproducción compatible en copias, sin atribución concluyente de su causa original. El recorrido posterior de escalado no repite esa comparación ni atribuye actividad de otros clientes. |
| Windows 125 % y 150 % | **Comprobado el 08/09/2026:** monitor 2 de 3440 × 1440, porcentajes aplicados en Windows, Chrome visible al 100 % y sin emulación. Cinco secciones, formularios y detalles existentes, sin desbordamiento global ni errores JavaScript detectados; tablas con desplazamiento interno, controles alcanzables y foco visible. No hubo peticiones de escritura API; Laboratorio se revisó sin nueva ejecución. **Restaurado y verificado el 100 % inicial** y cerrado el perfil temporal. [Evidencia y límites](validacion_escalado_windows.md). El intento anterior bloqueado y los 35 viewports CSS son antecedentes distintos. |
...
| CI de rc.2 | [Ejecución 34249730107](https://github.com/Buzo500/atlas-quant/actions/runs/34249730107), **superada** sobre `412918b5e9067e44f293b0633068ca932a472d64`: 446 Python + 91 subtests (22,13 s), 140 Vitest en 15 archivos (43,86 s), 5/5 E2E (18,7 s), instalación limpia, build, contratos, TypeScript, lint, `pip check`, smoke, arranque y parada correctos. E2E `e2e-ea578aac47f341ffac48cd6330edbb5a`: resultado 0, conservación verificada, integridad `ok` y puertos libres. |
