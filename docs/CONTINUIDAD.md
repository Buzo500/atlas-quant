...
**Publicación de la revisión autorizada el 09/09/2026:** el usuario pide subir los cambios y ejecutar la CI gratuita. Cuota comprobada en GitHub antes del envío: 50/2.000 minutos, 0/0,5 GB de almacenamiento y 0 USD facturables. Se conserva el workflow manual de Windows, con un único job estándar limitado a 20 minutos y sin artefactos remotos. El resultado remoto se registrará al terminar; esta autorización no pide fusionar la PR ni publicar una etiqueta.
...
Los ajustes de fichas, navegación y esta revisión permanecen **locales, sin commit ni subida**, sobre `0a5b813`, rama `codex/v0.3-graficos`, versión `0.3.0-dev.1`. No se ha ejecutado CI ni modificado motor/contratos/dependencias; no atribuir las pruebas Python históricas a una repetición actual. El usuario ha pedido terminar esta revisión y esperar sus siguientes instrucciones. G6 sigue abierto por la incidencia de API, la CI y la comprobación física de los controles actuales. Ensayo de 48 horas y seguimiento aplazados; aprendizaje, indicadores, LaTeX, remoto, móvil y bróker siguen fuera del alcance actual.
