...
**Primera CI de rc.2 fallida; PR #2 abierta:** `dff5e24a0ebbce8a1cb0481fd64e47563794e433` está subido a `Buzo500/atlas-quant`. La [CI 34248790750](https://github.com/Buzo500/atlas-quant/actions/runs/34248790750) falló sobre ese SHA: **444 pruebas Python superadas, dos fallidas y 91 subtests** en 21,99 s; las **140 pruebas Vitest pasaron**. Los dos fallos son las variantes del test de aislamiento durante colección: comparaba textualmente `store.path`, con el alias corto de Windows `RUNNER~1`, y una ruta resuelta con `runneradmin`, aunque identifican el mismo archivo. Se prepara una corrección limitada a esa aserción para repetir CI. Este run no acredita E2E remoto; el resultado E2E verde descrito abajo es local. Se conservan sus logs y el resultado fallido.

La [PR #2](https://github.com/Buzo500/atlas-quant/pull/2) permanece abierta; no se ha fusionado ni publicado la etiqueta rc.2. La autorización del usuario para la CI gratuita, la fusión y la etiqueta continúa vigente, con las dos últimas condicionadas a superar CI. El código inicial está en `e64351c0eb93ce2307be04ce31559c8aa9f3d24b`; `dff5e24` añade documentación. El rechazo previo de la revisión automática se conserva como antecedente, resuelto mediante autorización expresa.

**ATLAS está detenido antes de corregir el test.** El arranque anterior, ejecución `0e3cebcbaa5344b0ade7a17071b986bd`, había verificado versión rc.2, manifiesto, motor, proxy y HTML, conservando datos, auditoría previa, tres posiciones y NAV 25.118,66876 EUR, con parada global activada y consumo cero. Su evidencia se conserva en `output/validation/rc2-runtime-check.json`, `rc2-runtime-status.json` y `rc2-preservation.json`; no acredita que siga ejecutándose ahora. El escalado físico permanece sin validar y aplazado.
...
La siguiente ejecución de CI, la fusión y la etiqueta se registrarán cuando se confirmen; ninguna está completada por esta corrección en preparación. La etiqueta queda condicionada a superar CI y no se declara una versión estable. El ensayo sostenido de 48 horas y su seguimiento permanecen aplazados, con presupuesto cero, sin claves ni llamadas pagadas.
