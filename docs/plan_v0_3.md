...
**Estado de entrega:** G1–G5 implementadas en `0.3.0-dev.1`; G6 conserva la validación de la entrega inicial y recibe una [revisión de los controles actuales](revision_graficos_20260909.md). Pasan 244 pruebas frontend y los cinco recorridos E2E de gráficos, con nuevas medidas de rendimiento. La suite general queda en 9/10 por una espera intermitente de `/api/state`; siguen pendientes su diagnóstico, la CI de la rama y repetir el escalado físico sobre los controles nuevos. Los viewports CSS comprobados no sustituyen Windows 125 %/150 %. La [guía de gráficos](graficos_v0_3.md) registra uso y limitaciones; G6 no se da por cerrado ni se etiqueta estable.
