...
**Cierre de la entrega de desarrollo · 09/09/2026:** G1–G6 aceptadas por el usuario para `0.3.0-dev.1`. Pantalla completa confirmada manualmente en precios y cartera; timeout aceptado como incidencia conocida abierta. CI final `34346311068` sobre `d5c3b09`: 482 Python + 91 subtests, 244 frontend y 10 E2E. Fusión de PR #3 y etiqueta de desarrollo autorizadas. Este cierre sustituye los estados «en curso» y «G6 abierto» conservados a continuación como historial del plan. No declara estable v0.2 ni reactiva el ensayo aplazado. [Aceptación y evidencia](revision_final_v0_3.md).

Actualizado: 9 de septiembre de 2026. **Implementación autorizada y en curso.** Este documento fija el alcance y los criterios; no acredita por sí mismo que las funciones estén terminadas. El punto de partida es `v0.2.0-rc.2` (`9aee4224a12556e97cde16ed283854af10845bda`). La evidencia de cada entrega debe registrar el código realmente probado.

**Estado de entrega:** G1–G5 implementadas en `0.3.0-dev.1`; G6 recibe una [revisión de los controles actuales](revision_graficos_20260909.md) y la [CI gratuita 34340199451 superada](https://github.com/Buzo500/atlas-quant/actions/runs/34340199451) sobre `c8f4eb6`: 477 pruebas Python y 91 subtests, 244 frontend y 10/10 E2E. Hay nuevas medidas locales de rendimiento y [seis recorridos de escalado físico Windows 125 %/150 %](escalado_controles_20260909.md) correctos sobre precios, cartera y backtest, con 100 % inicial restaurado. La vista ampliada y sus gestos se comprobaron; la ocultación del marco de Chrome en uso normal queda sin certificar por el comportamiento del navegador automatizado descrito en esa evidencia. El intento local previo conserva su resultado 9/10 por una espera intermitente de `/api/state`; el [diagnóstico](diagnostico_api_20260909.md) no la reprodujo y su causa sigue abierta. La CI correcta no demuestra resuelta esa causa. La [guía de gráficos](graficos_v0_3.md) registra uso y limitaciones; G6 no se da por cerrado ni se etiqueta estable.
...
**Revisión vigente de aceptación:** [matriz G1–G6](revision_final_v0_3.md#revisión-de-los-tres-puntos-solicitados--09092026), posterior a la CI final `34346311068` sobre `d5c3b09` (482 Python + 91 subtests, 244 frontend, 10 E2E). G1–G5 y las comprobaciones técnicas de G6 tienen evidencia. El usuario confirmó manualmente el 09/09/2026 la pantalla completa en precios y cartera, ocultación del marco de Chrome y restauración con Escape; sustituye el pendiente del resumen histórico anterior. G6 sigue abierto por la decisión de aceptar el timeout como incidencia conocida en una entrega de desarrollo. No se repiten pruebas genéricas de API sin hipótesis nueva ni se declara resuelta la causa.

La [preparación de la revisión final](revision_final_v0_3.md) reúne la evidencia vigente, la validación de las herramientas de diagnóstico y los pendientes de G6. Preparar esa revisión no equivale a fusionar la PR ni publicar una versión.
