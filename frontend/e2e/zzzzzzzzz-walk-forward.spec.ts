...
test('v0.6 walk-forward: ventanas, criterios congelados, reserva y reproducción', async ({
  ...
  await expect(
    walkForward.getByText('0 de 3 ventanas cumplen · 3 evaluables.'),
  ).toBeVisible();
  await walkForward
    .getByText('Detalle de la ventana 1', { exact: true })
    .click();
  await expect(
    walkForward
      .getByRole('region', { name: 'Evaluación de la ventana 1', exact: true })
      .getByRole('cell', { name: '800,00 €', exact: true }),
  ).toBeVisible();
  ...
  for (const window of report.walk_forward!.windows) {
    expect(window.evaluation.metrics.map((m) => m.final_nav_eur)).toEqual(
      reference.expected_nav_eur,
    );
    expect(window.evaluation.end_date < report.protocol.holdout_date).toBe(
      true,
    );
  }
  expect(report.walk_forward!.unused_sessions).toBe(reference.unused_sessions);
  expect(report.walk_forward!.config.context_sessions).toBe(7);
});
